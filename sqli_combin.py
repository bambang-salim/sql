...
class MLComponent:
    ...
    def _initialize_scaler(self):
        ...
        self.scaler.fit(f)  # Using global feature data
        
        # Train models with initial data
        dummy_labels = np.random.randint(0, 2, len(f))
        for model in self.models.values():
            model.fit(self.scaler.transform(f), dummy_labels)
    ...
    def _save_results(self, process_id: int, results: list):
        ...
        with open(json_filename, 'w') as f:
            json.dump({
                ...
                'results': results,
                'mean_blocked': np.mean(results),
                'max_blocked': np.max(results),
                'min_blocked': np.min(results)
            }, f, indent=2)
